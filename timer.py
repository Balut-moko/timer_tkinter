...
class Timer(tk.Frame):
    ...
    def create_widgets(self):
        self.spinbox_min = ttk.Spinbox(self, from_=0, to=99, font=('Helvetica', 20), command=self.spin_changed)
        self.spinbox_min.set(self.minutes)
        self.spinbox_min.grid(row=0, column=0, sticky=tk.W)
        self.spinbox_sec = ttk.Spinbox(self, from_=0, to=59, font=('Helvetica', 20), wrap=True, command=self.spin_changed)
        self.spinbox_sec.set(self.seconds)
        self.spinbox_sec.grid(row=0, column=1, sticky=tk.W)
        self.start_button = tk.Button(self, bg="Red", activebackground="Dark Red", font=('Helvetica', 20), command=self.start)
        self.start_button.config(text="Start")
        self.start_button.grid(row=1, column=0, sticky=tk.W + tk.E + tk.N + tk.S)
        self.pause_button = tk.Button(self, bg="Green", activebackground="Dark Green", font=('Helvetica', 20), command=self.pause)
        self.pause_button.config(text="Pause")
        self.pause_button.grid(row=1, column=1, sticky=tk.W + tk.E + tk.N + tk.S)
        ...
        self.reset_button.grid(row=1, column=2, sticky=tk.W + tk.E + tk.N + tk.S)
        self.tokei = tk.Label(self, font=('Helvetica', 100), textvariable="")
        self.tokei.config(text="00:00")
        self.tokei.grid(row=2, column=0, columnspan=3)
